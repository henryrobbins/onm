import os
import pytest
from onm.sources import Sources, SourceType
...
def test_sources_get_plaid_source():
    sources_path = \
        os.path.join(os.path.dirname(__file__), 'resources/sources.ini')
...
def test_sources_add_plaid_source():
    sources_path = \
        os.path.join(os.path.dirname(__file__), 'resources/test-sources.ini')
    sources = Sources(sources_path)
    source = PlaidSource(
        name="onm_bank",
